        ...
        <div className="flex flex-row items-start">
          ...
          <div className="ml-3 mt-2 flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight">{store.name || 'Store Name'}</h2>
              {store.is_verified && (
            ...
            <p className="text-gray-500 text-sm mt-1">
              @{storeUsername} • {listings.length} mahsulot{store.subscriber_count !== undefined && store.subscriber_count > 0 && ` • ${store.subscriber_count} obunachi`}
            </p>
          </div>
        </div>
